...
@router.post("/payments/initiate/{member_id}/{period_id}", response_class=RedirectResponse)
async def initiate_payment(
    ...
    period = DuesPeriodService.get_by_id(db, period_id)
    if not period:
        raise HTTPException(status_code=404, detail="Period not found")

    # Get member's rate for this classification
    rate = DuesFrontendService.get_rate_for_member(db, member_id)
    if not rate:
    ...
    try:
        checkout_url = PaymentService.create_dues_checkout_session(
            db=db,
            member_id=member_id,
            period_id=period_id,
            amount=amount,
