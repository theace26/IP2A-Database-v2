...
def stress_test_members(db: Session, count: int = 10000):
    ...
    for i in range(count):
        ...
        member_data = {
            ...
            "middle_name": fake.first_name() if fake.boolean(chance_of_getting_true=45) else None,
            ...
            "state": fake.state_abbr() if fake.boolean(chance_of_getting_true=82) else None,
            "zip_code": fake.zipcode() if fake.boolean(chance_of_getting_true=78) else None,
            "phone": fake.phone_number()[:50] if fake.boolean(chance_of_getting_true=85) else None,
            "email": fake.email() if fake.boolean(chance_of_getting_true=65) else None,
            "date_of_birth": date_of_birth,
            "hire_date": hire_date,
            "status": status,
            "classification": classification,
            "notes": fake.sentence() if fake.boolean(chance_of_getting_true=15) else None,
        }
    ...
    print(f"   Adding members to database (this will take a moment)...")
